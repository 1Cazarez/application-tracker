...
How it works
    ...
    Upload — On /upload you pick a screenshot of a job posting. (Or skip straight to step 3 and type the details in by hand.)
    Extract — The image is POSTed to /api/extract, which sends it to Gemini 2.5 Flash with a prompt asking for a strict JSON object (company, title, deadline, pay, location, url, job_type).
    Confirm — Every extracted field renders as an editable input, so you fix whatever the model got wrong before committing it.
    Save — The row lands in the Supabase jobs table with status applied.
    Track — /dashboard lists your applications with counts per status, and flags any deadline within 3 days in red. Status is a dropdown on each card: applied → interview → offer → rejected.
    Remind — A Vercel cron hits /api/reminders daily at 13:00 UTC. For each user who has reminders on, it finds their non-rejected jobs falling inside their chosen lead time that haven't been mailed yet, sends one digest via Resend in their language, and marks them reminder_sent.
